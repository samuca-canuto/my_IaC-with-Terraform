...
user_data = <<-EOF
  ...
  cd /tmp
  wget -q https://github.com/open-telemetry/opentelemetry-collector-releases/releases/download/v0.131.1/otelcol-contrib_0.131.1_linux_amd64.deb

  dpkg -i otelcol-contrib_0.131.1_linux_amd64.deb || apt -f install -y
